...
class Linear_Map(Map):
    ...
    def shape_function(self, x,y):
        ...
        matrix =    [[x,y,1],
                     ...
                     [self.c[0], self.c[1], 1]]
        matrix = np.array(matrix)
        return np.linalg.det(matrix) / self.area()

    def gradient_function(self, x, y):
        dx = self.b[1] - self.c[1]
        dy = -self.b[0] + self.c[0]
        return np.array([dx, dy] / self.area())
    
    def integral(self):
        return self.area() * 1/3
...
class Linear_Integrator(Integrator):
    ...
    def mixed_gradient_integral(self) -> np.ndarray:
        df1_df2 = np.kron(self.map1.gradient_function(1,1) , self.map2.gradient_function(1,1)) * np.linalg.norm(self.map1.area())
        return df1_df2.reshape((2,2))
    def mixed_value_grad_integral(self):
